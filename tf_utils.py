...
def read_and_decode(filename_queue, batch_size = 8, capacity=30, num_threads=1, min_after_dequeue=10, is_training=True):
    ...
    image = tf.decode_raw(features['image/image_data'], tf.float32)
    mask_class = tf.decode_raw(features['image/mask_class'], tf.uint8)
    mask_instance = tf.decode_raw(features['image/mask_instance'], tf.uint8)#!!!format must be the same as the raw data.
    image = tf.reshape(image, [512, 512, 1])
    #image = tf.cast(image,tf.float32)
    #mask_class=tf.reshape(mask_class, [512, 512, 1])
    
    mask_class=tf.reshape(mask_class, [512, 512])
    mask_class = tf.cast(mask_class,tf.int32)
    mask_instance= tf.reshape(mask_instance, [512, 512, 1])
    ...
    if is_training is True:
        ...
        image,mask_class,mask_instance = tf.train.batch([image,mask_class,mask_instance], batch_size=batch_size,\
...
def read_and_decode_for_lstm(filename_queue, batch_size = 8, capacity=30, num_threads=1, min_after_dequeue=10, is_training=True):
    ...
    features = tf.parse_single_example(
      ...
      features={
        ...
        'image/channels': tf.FixedLenFeature([], tf.int64),       
        'image/image_data': tf.FixedLenFeature([], tf.string),
        'image/superpixels': tf.FixedLenFeature([], tf.string),
        'image/mask_class': tf.FixedLenFeature([], tf.string),
        'image/mask_instance': tf.FixedLenFeature([], tf.string)
        })
    height = tf.cast(features['image/height'], tf.int64)
    width = tf.cast(features['image/width'], tf.int64)
    image = tf.decode_raw(features['image/image_data'], tf.int16)
    superpixles = tf.decode_raw(features['image/superpixels'], tf.int64)
    mask_class = tf.decode_raw(features['image/mask_class'], tf.uint8)
    mask_instance = tf.decode_raw(features['image/mask_instance'], tf.uint8)#!!!format must be the same as the raw data.
    image = tf.reshape(image, [512, 512, 1])
    image = tf.cast(image,tf.float32)
    superpixles=tf.reshape(superpixles, [512, 512])    
    mask_class=tf.reshape(mask_class, [512, 512])
    mask_class = tf.cast(mask_class,tf.int32)
    mask_instance= tf.reshape(mask_instance, [512, 512, 1])
    ...
    if is_training is True:
        ...
        image,mask_class,superpixles = tf.train.shuffle_batch( [image,mask_class,superpixles], batch_size=batch_size,
                                                 capacity=capacity,
                                                 num_threads=num_threads,
                                                 min_after_dequeue=min_after_dequeue)
    else:
        image,mask_class,superpixles = tf.train.batch([image,mask_class,superpixles], batch_size=batch_size,\
                                                        num_threads=num_threads,capacity=capacity)
    return image, mask_class, superpixles
